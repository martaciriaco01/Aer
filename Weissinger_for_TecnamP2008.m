...
rho = 1.225;
config.NCorpi = 1;
config.RootChord = 1.373; 
config.DihedralAngle = 1;      
config.SweepAngle = 0;        
config.TaperRatio = 0.8;       
config.AspectRatio = 6.7;
config.Span = 9;
...
for iCorpo = 1:config.NCorpi
    for sweep_idx = 1:3
        config.SweepAngle = 30*(sweep_idx-1);
        ...
        for ChordPanel_i = 1:config.ChordwiseDiscr(iCorpo)
            for SpanPanel_i = 1:2*config.SemiSpanwiseDiscr(iCorpo)
                ...
                V_local = U_Inf * cosd(config.SweepAngle(iCorpo));
                V_magnitude = norm(V_local);
                
                % Applicazione Kutta-Joukowski
                Lift3D = Lift3D + rho * V_magnitude * GammaHere * dSpan;
            end
        end
    end
end
fprintf('3D Lift: %.4f N\n', Lift3D);

%% Compute 2D Lift distribution
Lift2D = cell(config.NCorpi, 1);
for iCorpo = 1:config.NCorpi
    Lift2D{iCorpo} = rho * U_Inf(1)  * sum(Gamma{iCorpo}, 1) * config.MAC(iCorpo);
end
...
%% Calcolo della portanza 2D e 3D
Lift3D = 0;
for iCorpo = 1:config.NCorpi
    SpanPoints = linspace(-config.SemiSpan(iCorpo), config.SemiSpan(iCorpo), 2 * config.SemiSpanwiseDiscr(iCorpo));
    GammaDistribution = sum(Gamma{iCorpo}, 1);
    Lift3D = Lift3D + trapz(SpanPoints, rho * U_Inf_Mag * GammaDistribution);
end
Lift2D = rho * U_Inf_Mag * Gamma{iCorpo};
...
figure;
plot(alpha_range, CL_values, 'm-o');
xlabel('Alpha(gradi)');
ylabel('C_L');
title('CL/ALPHA');
grid on;
...
figure;
plot(alpha_range, CD_values, 'm-o');
